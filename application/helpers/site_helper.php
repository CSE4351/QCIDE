...
if(!function_exists('lookup_user')){
    function lookup_user($uid_or_username = null,$fail_message = 'User lookup failed.',$throw_error = true){
        ...
        if(!empty($user)){
            $user['id'] = $user['uid'];
            $user['poster'] = json_decode($user['poster'],true);
        }
        ...
        foreach($hash as $item){
            ...
            foreach($item['keys'] as $key){
                ...
                if(array_key_exists('min',$item)){
                    if(floatval($array[$key]) < $item['min']){
                        throw new Exception('The minimum value for key `' . $key . '` is ' . $item['min']);
                    }
                }
                if(array_key_exists('max',$item)){
                    if(floatval($array[$key]) > $item['max']){
                        throw new Exception('The maximum value for key `' . $key . '` is ' . $item['max']);
                    }
                }
            }
        }
...
if(!function_exists('s3_name')){
    function s3_name($prefix,$file_name,$appendix = '', $uniq_id = null){
        $ext = ext($file_name);
        $no_ext_file_name = str_replace('.' . $ext, '', $file_name);
        return $prefix . ($uniq_id ?: uniq_hash()) . '_' . sluggify($no_ext_file_name,true) . $appendix . '.' . $ext;
    }
}
if(!function_exists('files_url')){
    function files_url($bucket, $url){
        return str_ireplace('https://s3.amazonaws.com/' . $bucket . '/', FILES_URL, $url);
    }
}
if(!function_exists('static_url')){
    function static_url($bucket, $url){
        return str_ireplace('https://s3.amazonaws.com/' . $bucket . '/', STATIC_URL, $url);
    }
}
...
if(!function_exists('curl')){
    function curl($url){
        ...
        $data = curl_exec($ch);
        if($data === false){
            throw new Exception('Error fetching URL: ' . $url . ' - ' . curl_error($ch) . ' - Code: ' . curl_errno($ch));
        }
        $rcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);

        if (stripos($rcode, '40') !== false || stripos($rcode, '50') !== false){
...
if(!function_exists('custom_redirect')){
    function custom_redirect($hash){
        $hash = default_array($hash, array(
            ...
            array(
                'keys' => array('uri','message','header'),
                'value' => ''
            )
        ));
        header('Location: ' . BASE_URL . $hash['uri'] . (!empty($hash['header']) ? '?' .
                http_build_query(
                    [
                        'header' => $hash['header'],
