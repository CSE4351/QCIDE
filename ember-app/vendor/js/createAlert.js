...
function createAlert(title,body,type,sticky,where,input_options){
    ...
    }else if(type === 'danger' || type === 'error' || type === 'failure'){
...
function createError(title, hash, long){
    long = typeof long === 'undefined' ? false : long;
    var message = '';
    if(typeof hash === 'string'){
        message = hash;
    }else if(hash && hash.errors && hash.errors[0]){
        if(hash.errors[0].detail && hash.errors[0].detail.error){
            message = hash.errors[0].detail.error;
        }else if(hash.errors[0].message){
            message = hash.errors[0].message;
        }
    }
    if(message === ''){
        message =  hash && hash.message ? hash.message : 'Server error';
    }
    createAlert(title,message,'danger', long);
}
function showLoader(){
    $('.toast').remove();
...
function createConfirm(options){
    ...
    if(typeof options.buttons === 'object'){
        bootbox_options['buttons'] = options.buttons;
